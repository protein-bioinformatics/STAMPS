...
if sys.argv[1] == sys.argv[3]:
    print("input_blib_1 and output_blib must be different files")
    exit()
    
if sys.argv[2] == sys.argv[3]:
    print("input_blib_2 and output_blib must be different files")
    exit()
...
cur_out = db.cursor()
cur_data = db.cursor()
cur_in1.execute("ATTACH DATABASE '%s' As second;" % sys.argv[2])

#check for new schemes
try:
    cur_out.execute("select count(driftTimeMsec) from RefSpectra;")
except:
    print("Error: %s has old or unrecognizable scheme. Exit" % sys.argv[1])
    exit()
try:
    cur_out.execute("select count(driftTimeMsec) from second.RefSpectra;")
except:
    print("Error: %s has old or unrecognizable scheme. Exit" % sys.argv[2])
    exit()
...
cur_out.execute("ATTACH DATABASE '%s' As merged;" % sys.argv[3])
...
print("library 1 contains %i spectra" % len_spectra)
print("library 2 contains %i spectra" % second_count)
...
print("inner join contains %i spectra" % join)
print("taken %i in join of first lib" % (join - score2_taken))
print("taken %i in join of second lib" % score2_taken)
